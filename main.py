...
def scan_symbol(name):
    ...
    try:
        ...
        return {
            ...
            "confidence": conf, "regime": regime, "strategy": strategy,
            "close": round(close, 2), "atm_strike": atm_strike, "otm_strike": otm_strike,
            "atm_prem": atm_prem, "sl_prem": sl_prem, "tgt_prem": tgt_prem,
            "sugg_lots": sugg_lots, "lot": lot,
            "sl_idx": round(sl_idx, 2), "tgt_idx": round(tgt_idx, 2),
            "atr": round(atr, 2), "rsi": round(rsi, 1),
            "ema9": round(ema9, 2), "ema21": round(ema21, 2), "vwap": round(vwap, 2),
            "dte": dte, "expiry_today": is_expiry_today(name), "yahoo": cfg["yahoo"],
        }
    except Exception as e:
